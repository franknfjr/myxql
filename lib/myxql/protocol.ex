defmodule MyXQL.Protocol do
  ...
  def decode_com_query_response(<<0xFB, rest::binary>>, "", :initial) do
    IO.puts("DEBUG: Received LOCAL INFILE request")
    IO.puts("DEBUG: Rest binary: #{inspect(rest)}")

    {filename, _remaining} = take_string_nul(rest)
    IO.puts("DEBUG: Parsed filename: #{inspect(filename)}")
  ...
  defp decode_resultset(<<0xFB, rest::binary>>, _next_data, :initial, _row_decoder) do
    IO.puts("DEBUG: Received LOCAL INFILE request in resultset")
